...
<template>
    <div class="hui-datepicker" :style="`width: ${width}px;`">
        <VueDatePicker locale="zh-CN" :month-change-on-scroll="false" :enable-time-picker="showTime" :time-picker="time"
            :week-picker="week" :month-picker="month" :year-picker="year" now-button-label="今天"
            :show-now-button="showToday" auto-apply text-input :model-type="modelType"
            :day-names="['一', '二', '三', '四', '五', '六', '七']" v-bind="$attrs"></VueDatePicker>
    </div>
</template>
...
:deep(.dp__theme_dark) {
    ...
    --dp-primary-color: var(--brand_blue);
...
:deep(.dp__theme_light) {
    ...
    --dp-border-color-hover: var(--brand_blue);
    --dp-border-color-focus: var(--brand_blue);
    ...
    .dp__input_focus {
        border: 1px solid var(--dp-border-color-focus) !important;
    }
