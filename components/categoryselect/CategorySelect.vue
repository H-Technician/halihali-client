        ...
        <div class="drop-container" ref="categoryPop" :style="isShowPop ? '' : 'display: none;'">
            ...
            <div class="drop-sub" v-if="categoryList.length != 0">
                ...
                >
                    ...
                    >{{ item.subName }}</p>
                    <p class="item-descr" v-if="item.subDescr">{{ item.subDescr }}</p>
                </div>
            </div>
        </div>
    </div>
...
const selectSub = (subIndex: number) => {
    ...
    categorySub.value = categoryList.value[main.value].subCategories[subIndex].subName;
    const categoryId = categoryList.value[main.value].id;
    const categorySubId = categoryList.value[main.value].subCategories[subIndex].subId;
    isShowPop.value = false;
    emit("SelectCategory", categoryId, categorySubId, categoryList.value[main.value].subCategories[subIndex].subTag);     // 将分区更新回父组件
}
