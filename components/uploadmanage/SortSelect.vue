<template>
    <div class="select-box">
        <div class="select-controller"
            ref="selectController"
            :class="isShowPop ? 'select-controller-selecting' : ''"
            @click="isShowPop = !isShowPop"
        >
            <p :class="isSelected ? 'select-item-inserted' : 'select-item'">{{ selectMainText }}</p>
            <IconsArrowDownBold class="icon-arrowdown" :class="isShowPop ? 'selecting-icon' : ''"/>
        </div>
        <div class="drop-container" ref="selectPop" :style="isShowPop && sortList.length != 0 ? '' : 'display: none;'">
            <div class="drop-main">
                <div v-if="sortList.length != 0">
                    <div
                        class="drop-main-item"
                        :class="main == index ? 'drop-main-item-selected' : ''"
                        v-for="(item, index) in sortList"
                        :key="index"
                        @click="selectMain(index)"
                    >
                    {{ item.item }}
                    <IconsSelect 
                    class="icon-select" 
                    style="margin-left: 6px;" 
                    v-if="main == index"/>
                    </div>
                </div>
            </div>
        </div>
    </div>
</template>
<script setup lang="ts">
const selectMainText = ref("");
const isShowPop = ref(false);
const isSelected = ref(false);
const main = ref(0);    // 选中主分区在authList中的index 控制pop的显示
const emit = defineEmits(['SelectAuth']);
const selectController = ref<HTMLElement | null>(null);
const selectPop = ref<HTMLElement | null>(null);
// 点击空白处关闭气泡
const handleOutsideClick = (event: { target: any; }) => {
    if (selectController.value && selectPop.value) {
        if (
            !selectController.value.contains(event.target)
            && !selectPop.value.contains(event.target)
        ) {
            isShowPop.value = false;
        }
    }
};
const sortList = ref([
    { item: "投稿时间排序" },
    { item: "播放数排序" },
    { item: "收藏数排序" },
    { item: "弹幕数排序" },
    { item: "评论数排序" },
]);
// 点击创作者声明
const selectMain = (mainIndex: number) => {
    if (!isSelected.value) {
        isSelected.value = true;
    }
    main.value = mainIndex;
    selectMainText.value = sortList.value[main.value].item;
    emit("SelectAuth", sortList.value[main.value].item);
    isShowPop.value = false;
};

onMounted(async () => {      
    // 页面渲染后创建点击事件的监听器
    window.addEventListener("click", handleOutsideClick);
    selectMainText.value = sortList.value[main.value].item;
})
onBeforeUnmount(() => {
    // 关闭页面前清除点击事件的监听器
    window.removeEventListener("click", handleOutsideClick);
})

</script>

<style scoped lang="scss">
.select-box {
    position: relative;
    -webkit-user-select: none;
    -moz-user-select: none;
    -ms-user-select: none;
    user-select: none;
    min-width: 170px;
    height: 40px;
    box-sizing: border-box;

    .select-controller {
        box-sizing: border-box;
        padding: 7px 12px;
        border: 1px solid #dcdfe6;
        border-radius: 4px;
        min-width: 170px;
        display: inline-flex;
        justify-content: space-between;
        align-items: center;
        cursor: pointer;
        color: #000;
        transition: border .3s ease;
        background: #fff;

        .select-item-inserted {
            color: #222;
            line-height: 22px;
            height: 22px;
            font-size: 14px;
            display: inline-block;
            white-space: nowrap;
            margin: 0;
        }

        .select-item {
            color: #000;
            line-height: 22px;
            height: 22px;
            font-size: 14px;
            display: inline-block;
            white-space: nowrap;
            margin: 0;
        }

        &:hover {
            border-color: var(--brand_blue);
        }

        .selecting-icon {
            transform: rotate(180deg);
        }

        .icon-arrowdown {
            transition: transform 0.3s;
            margin-left: 4px;
            width: 14px;
            height: 14px;
        }
    }

    .select-controller-selecting {
        border-color: var(--brand_blue);
    }

    .drop-container {
        background: #fff;
        border: 1px solid #e5e9ef;
        box-shadow: 0 2px 4px 0 rgba(0,0,0,.14);
        border-radius: 4px;
        position: absolute;
        left: 0;
        top: 46px;
        z-index: 10;
        display: flex;

        .drop-main {
            padding: 12px 0;
            width: 170px;
            height: auto;
            overflow-x: hidden;
            overflow-y: auto;

            .drop-main-item {
                padding: 10px 24px;
                width: auto;
                cursor: pointer;
                display: flex;
                justify-content: start;
                align-items: center;
                color: #99a2aa;
                font-size: 15px;

                &:hover {
                    background-color: #f4f5f7;
                }

                .icon-select {
                    width: 14px;
                    height: 14px;
                }
            }

            .drop-main-item-selected {
                color: var(--brand_blue);
                .icon-select {
                    fill: var(--brand_blue);
                }
            }
        }

    }
}
</style>