...
const emit = defineEmits(['selectedBackrate', 'isShowMenu']);
// 播放速度列表
...
const selectedBackrate = (index: number) => {
    if (isSelectedBackrate.value === index) return;
    isSelectedBackrate.value = index;
    emit('selectedBackrate', playbackrateList.value[index].backrate, playbackrateList.value[index].value);
    emit('isShowMenu', false);
}
</script>
