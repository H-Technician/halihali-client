...
interface Props {
  ...
  modelValue?: string // (v-model) 输入框内容
...
const emits = defineEmits(['update:modelValue', 'change', 'enter'])
...
<template>
  <div class="hui-input-wrap" :style="`width: ${inputWidth};`">
    ...
    >
      ...
      <span v-if="showInputSuffix" class="input-suffix">
        <span v-if="showClear" class="m-actions" :class="{ 'clear-hidden': !props.modelValue }" @click="onClear">
          <svg
            class="clear-svg"
            focusable="false"
